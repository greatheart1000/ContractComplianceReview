<template>
    <div class="EditRuleInfo">
        ...
        <div style="text-align:center; margin-top:30px;overflow:hidden;width:100%;height:50px;">
            <div v-if="$yqvutil.getVerifyAuthority('compliance.contract_standard.update')" style="margin-left:calc(50% - 36px)" class="new" @click="saveContractRule">
  ...
  export default {
    ...
    methods: {
      ...
      getContractRule(ruleId) {
        let vm = this;
        vm.$http.post(vm.$yqvutil.getContextPath()+`yqvservice/cstandard/find`,{"ruleId":ruleId})
      ...
      saveContractRule() {
        ...
        if (!this.ruleTypeVerify && !this.ruleNameVerify && !this.contractTypesVerify && !this.checkContentVerify && !this.solveSuggestVerify) {
          let message = "新增成功";
          let url = vm.$yqvutil.getContextPath()+"yqvservice/cstandard/insert";
          if (vm.ruleData.ruleId) {
            url = vm.$yqvutil.getContextPath()+"yqvservice/cstandard/update";
            message = "修改成功";
          }
          vm.$http
              .post(url, data)
